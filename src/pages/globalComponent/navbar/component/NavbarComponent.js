    ...
    <AppBar position="static" style={{ background: '#2E3B55' }}>
      <Container maxWidth="xl">
        <Toolbar disableGutters>
          ...
          <Box sx={{ flexGrow: 0 }}>
              {!data.userInfo?<><Button onClick={() => navigate('/login')}>Login</Button> | <Button onClick={() => navigate('/register')}>register</Button>  </>: <Tooltip title="Open settings">
                {/* <Button></Button> */}
            ...
            >
              {settings.map((setting,index) => (
                <MenuItem key={setting} onClick={handleCloseUserMenu}>
                    {setting==='Logout'?<LogoutButton/> : 
                  <Typography textAlign="center" onClick={() => navigate(settingsLink[index])}>{setting}</Typography>}
                </MenuItem>
              ))}
            </Menu>
          </Box>
